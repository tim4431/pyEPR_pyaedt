...
class HfssProject(COMWrapper):
    ...
    def new_dm_design(self, name: str):
        ...
        On AEDT 2024.1+, ``InsertDesign`` with solution type ``"DrivenModal"``
        silently creates an ``"HFSS Hybrid Modal Network"`` design instead of
        the classic ``"HFSS Modal Network"``.  pyEPR explicitly calls
        ``SetSolutionType`` after creation to enforce the non-hybrid type,
        matching the behaviour of all previous HFSS releases.

        See Ansys scripting docs:
            InsertDesign  — cannot specify hybrid vs. non-hybrid at creation time.
            SetSolutionType — used post-creation to correct the type.

        Args:
            name (str): Name for the new driven-modal design.

        Returns:
            HfssDesign: Wrapper around the newly created HFSS design.
        """
        design = self.new_design(name, "DrivenModal")
        # AEDT 2024.1+ defaults InsertDesign to Hybrid; override to non-hybrid.
        if self._ansys_version >= "2024.1":
            design._design.SetSolutionType("HFSS Modal Network")
        return design

    def new_dt_design(self, name: str):
        """Create a new driven-terminal (non-hybrid) HFSS design.

        On AEDT 2024.1+, ``InsertDesign`` with solution type
        ``"DrivenTerminal"`` silently creates an
        ``"HFSS Hybrid Terminal Network"`` design instead of the classic
        ``"HFSS Terminal Network"``.  pyEPR explicitly calls
        ``SetSolutionType`` after creation to enforce the non-hybrid type,
        matching the behaviour of all previous HFSS releases.

        This is the terminal-port counterpart to :meth:`new_dm_design`.
        Use it when you need S-parameter or impedance extraction via
        terminal (lumped-port) excitations rather than wave-port modes.
        ...
        Returns:
            HfssDesign: Wrapper around the newly created HFSS design.
        """
        design = self.new_design(name, "DrivenTerminal")
        # AEDT 2024.1+ defaults InsertDesign to Hybrid; override to non-hybrid.
        if self._ansys_version >= "2024.1":
            design._design.SetSolutionType("HFSS Terminal Network")
        return design

    def new_em_design(self, name: str):
        """Create a new eigenmode HFSS design.

        Eigenmode designs compute the resonant frequencies and field
        distributions of a closed (or nearly closed) cavity without
        external ports.  No ``SetSolutionType`` workaround is needed
        because HFSS has never introduced a hybrid variant for Eigenmode.
    ...
    def new_q3d_design(self, name: str):
        ...
        Returns:
            HfssDesign: Wrapper around the newly created Q3D design.
        """
