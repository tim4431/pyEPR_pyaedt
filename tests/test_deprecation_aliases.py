...
class TestTopLevelAliases:
    """pyEPR_pyaedt.Project_Info etc. should warn and return the right class."""

    def test_project_info_alias_warns(self):
        import pyEPR_pyaedt
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            cls = pyEPR_pyaedt.Project_Info
        assert len(w) == 1
    ...
    def test_project_info_alias_returns_correct_class(self):
        ...
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always")
            cls = pyEPR_pyaedt.Project_Info
        assert cls is pyEPR_pyaedt.ProjectInfo

    def test_hfss_analysis_alias_warns(self):
        import pyEPR_pyaedt
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            cls = pyEPR_pyaedt.pyEPR_HFSSAnalysis
        assert len(w) == 1
        assert issubclass(w[0].category, DeprecationWarning)
    ...
    def test_hfss_analysis_alias_returns_correct_class(self):
        ...
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always")
            cls = pyEPR_pyaedt.pyEPR_HFSSAnalysis
        assert cls is pyEPR_pyaedt.DistributedAnalysis

    def test_quantum_analysis_alias_warns(self):
        import pyEPR_pyaedt
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            cls = pyEPR_pyaedt.pyEPR_Analysis
        assert len(w) == 1
        assert issubclass(w[0].category, DeprecationWarning)
    ...
    def test_quantum_analysis_alias_returns_correct_class(self):
        ...
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always")
            cls = pyEPR_pyaedt.pyEPR_Analysis
        assert cls is pyEPR_pyaedt.QuantumAnalysis

    def test_unknown_attribute_raises(self):
        import pyEPR_pyaedt
        with pytest.raises(AttributeError):
            _ = pyEPR_pyaedt.nonexistent_name_xyz


class TestCoreModuleAliases:
    """pyEPR_pyaedt.core.Project_Info etc. should warn and return the right class."""

    def test_project_info_alias_warns(self):
        from pyEPR_pyaedt import core
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
...
class TestCanonicalNamesNoWarning:
    ...
    def test_distributed_analysis_no_warning(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            _ = pyEPR_pyaedt.DistributedAnalysis
        dep_warnings = [x for x in w if issubclass(x.category, DeprecationWarning)]
        assert dep_warnings == []
    ...
    def test_quantum_analysis_no_warning(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            _ = pyEPR_pyaedt.QuantumAnalysis
        dep_warnings = [x for x in w if issubclass(x.category, DeprecationWarning)]
        assert dep_warnings == []
